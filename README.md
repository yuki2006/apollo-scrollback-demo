...
### 関係ない pagination の tail も掃除する

`useInfiniteQuery` は自分の Connection field を evict するだけでなく、**過去に訪れた他リストの tail も同じタイミングでまとめて捨てます**。

仕組み：

- 各 `useInfiniteQuery` インスタンスは data が確定したタイミングで自分の evict 記述子 (`{ id?, fieldName }`) を**モジュールスコープの registry** に登録
- PUSH/RELOAD 着地時は、自分の field を evict した上で、registry の他のエントリも全て evict

シナリオ例：

```
1. PostList を訪問 → posts(...) tail が 5 ページ分蓄積
2. PUSH で UserList へ → posts tail も users tail (まだ空) も registry 経由で掃除
3. PUSH で PostList に戻る → users tail はすでに evict 済みで、PostList 視点では noop
```

これがないと、別画面の古い tail が cache に潜在し続け、後で再訪したときに「先頭は最新、深いところは古い」状態が再発します。永続キャッシュ（`apollo3-cache-persist` 等）を使うときには特に効きます。

POP 着地時は registry を触らない（戻り側のキャッシュ復元を壊さない）よう、PUSH/RELOAD 限定の挙動です。
...
│   ├── paginationRegistry.ts     # 過去訪問した pagination の evict 記述子
