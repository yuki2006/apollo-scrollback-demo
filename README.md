...
|---|---|---|---|
| **POP**（戻る / 進む） | `useNavigationType()` | `cache-first` | 保持 |
...
ポイントは **「POP の時だけは絶対に再フェッチしない」**。これが守られて初めて、スクロール位置の復元が成立します。`cache-and-network` で裏側で再取得が走ると、リスト要素の高さが微妙に変わり、復元位置が数十ピクセルズレます。仮想化なしのリストではこのズレが致命的です。
...
│       components/ScrollManager.tsx        │  location.key 単位でスクロール保存・復元
├──────────────────────────────────────────┤
...
### 4. スクロール復元は `location.key` 単位で保存・POP のみ復元

```ts
const positions = new Map<string, number>();

// 保存: scroll イベントで throttle して書き込み + unmount 時に保存
// 復元: navKind === "POP" のときだけ requestAnimationFrame × 2 でデータ描画後に当てる
```

`location.key` は React Router が遷移ごとに一意な ID を振ってくれるので、保存単位として使えます。

`requestAnimationFrame` を二重に挟むのは、`cache-first` で同期的にデータが返っていても、レイアウト完了まで 1 フレーム必要なため。useLayoutEffect だけだと白画面で scrollTo が実行されて結局トップに戻ります。

**仮想化リストを使う場合はこの実装は使えません**。px 単位ではなくインデックス／アイテム ID で保存して `scrollToIndex` する必要があります。`ScrollManager` を差し替えるだけで `useInfiniteQuery` 側は変更不要、という分離になっています。
...
├── components/
│   ├── ScrollManager.tsx         # スクロール位置の保存・復元
